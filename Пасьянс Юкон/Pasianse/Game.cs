        ...
        public static void UpdateForm()
        {
            ...
            for (int i = 0; i < 7; i++)
            {
                for (int j = 0; j < UserView.NumberOfCardsInStacks[i]; j++)
                {
                    if (j >= i)
                    {
                        AllCards.cards[counter].PictureBox.Location = new Point(0, (j - i) * UserView.openedCardsDistance + OpenedCardsStart[i]);
                    }
                    else
                    {
                        AllCards.cards[counter].PictureBox.Location = new Point(0, j * UserView.closedCardsDistance);
                    }
                   AllCards.cards[counter].PictureBox.BringToFront();
                    counter++;
                }
            }
        }
